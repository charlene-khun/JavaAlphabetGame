...
public class Game extends JFrame implements ActionListener
{
	...
	//Used to generate random order of questions asked
	Random r = new Random();	
	...
	//Create set to hold unique values of visited questions
	...
	//ImageIcon array to hold images associated with 
	//each question
	ImageIcon[] image = 
	{
	...
	//More global variables with some private variables 
	//created from WindowBuilder
	JLabel name1;
	JLabel name2;
	JLabel thanks;
	...
	/* Global variables to hold players 
	 and their scores
	 */
	String player1;
	int score1;
	...
	public Game() {
		...
		//Create label with styles for window that 
		...
		// if user clicks cancel button or "X" button
		// from JOption Pane
		if (playerName == null)
		{
			UIManager.put("OptionPane.minimumSize",
			...
			thanks.setFont
			(new Font("Comic Sans MS", Font.BOLD, 35));
				...
				|| playerName.contains(" "))
        {
            ...
            ("OptionPane.background", new Color(224, 224, 224));
            UIManager.put
            ("Panel.background", new Color(224, 224, 224));
		...
		//Create multiple choice buttons for the user 
		//to choose from
		choice1 = new JButton();
		...
		//Create new label to hold the question's 
		//corresponding image
		pic = new JLabel("");
		...
		liveScore.setHorizontalAlignment
		(SwingConstants.CENTER);
		liveScore.setFont
		(new Font("Comic Sans MS", Font.PLAIN, 28));
		...
		EXIT2_Button.setFont
		(new Font("Comic Sans MS", Font.PLAIN, 28));
		EXIT2_Button.setBounds(37, 399, 201, 85);
		panel.add(EXIT2_Button);
		
		//Create How To Play button that 
		//will link to the How to Play page
		...
		HTP2_Button.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent e) 
			{
				...
				msg.setFont
				(new Font("Comic Sans MS", Font.PLAIN, 34));
				msg.setEditable(false);
				JOptionPane.showMessageDialog
				(frame, msg, "How to Play",
						JOptionPane.INFORMATION_MESSAGE);
			}
		});
		//Add button styles
		HTP2_Button.setFont
		(new Font("Comic Sans MS", Font.PLAIN, 24));
		...
		Alphabet_Learner.setFont
		(new Font("Comic Sans MS", Font.PLAIN, 26));
		Alphabet_Learner.setHorizontalAlignment
		(SwingConstants.CENTER);
		Alphabet_Learner.setBounds(0, 32, 267, 46);
		Green_Panel.add(Alphabet_Learner);
		
		/*
		 * Create leaderboard button. THis button will 
		 * take users who hold a top 5 score and add their 
		 * names and scores to the leaderboard.
		 * There are already 5 players who have "played" 
		 * the game by default and their scores have been 
		 * recorded to the leaderboard. Any new
		 * scores that out-score the current leaders will be 
		 * added to the leaderboard.
		 */
		LEADER_BOARD = new JButton("Leaderboard");
		LEADER_BOARD.setFont
		(new Font("Comic Sans MS", Font.PLAIN, 24));
	...
	//Method that will get a new question to be 
	//displayed to the game
	...
	public void actionPerformed(ActionEvent e)
	{
		//Add styles
		UIManager.put
		("OptionPane.minimumSize",new Dimension(400,100));
		UIManager.put
		("OptionPane.background", new Color(229,204,255));
		UIManager.put
		("Panel.background", new Color(229,204,255));
		...
		if(e.getSource()== choice1) 
		{
			...
			if(answer == answers[index]) 
			{
				...
				point++;	
				//Increment point if the user answers correctly
				//Update live counter
				liveScore.setText("Score: " + point);	
				nextQuestion();
			}
